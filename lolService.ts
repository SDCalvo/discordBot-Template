...
const la2URL = 'https://la2.api.riotgames.com';
const americasURL = 'https://americas.api.riotgames.com';
...
export async function lolApi(msg: Discord.Message) {
  ...
  const resSummoner = await axiosLoL.get(`${la2URL}/lol/summoner/v4/summoners/by-name/${summoner}`);
  //get account ID
  ...
  //get match history
  const resMatchs = await axiosLoL.get(`${americasURL}/lol/match/v5/matches/by-puuid/${puuid}/ids?start=0&count=20`);
